 		...
 		bxGallery: function(options) {
			...
			function preload_img(){
				...
				$orig.find('img').each(function(){
					...
					the_img.onload = preload_check;
					the_img.onerror = preload_check;
			...
			function set_main_img(){
				...
				$orig.width(wide);
				$orig.addClass('main_galery_image');
			...
			function place_thumbcontainer(){
				if(o.thumbplacement == 'top'){
					...
					$outer.prepend('<ul class="thumbs">' + cont + '</ul>');
					$orig.css({'float':'left'});
					$outer.find('.thumbs').css({'float':'left', 'height': (o.maxheight + 7), 'overflow': 'auto'});
