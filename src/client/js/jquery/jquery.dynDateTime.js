...
Changes made for echopoint:

- Pass in a reference to the echo object itself, so we can call the forceRedraw
  with the correct context to prevent a IE7 bug

- The echo object property _calObject is set to the created calendar object.
  We use this on the echo side to remove the calendar div from the dom tree.
  Otherwise we will have a lot of hidden calendar divs in the dom tree.
	...
	$.fn.dynDateTime = function(options) {
		...
		return this.each(function() {
			...
			if (this_flat != null) {
				...
				var cal = new Calendar(opts.firstDay, 
                                                        opts.date,
                                                        opts.onSelect || onSelect,
                                                        null,
                                                        opts.echoComponent
                                                        );
                                opts.echoComponent._calObject= cal;
...
// $Id: jquery.dynDateTime.js,v 1.1.1.1 2010-04-01 09:47:33 perxi Exp $

/** The Calendar object constructor. */
Calendar = function (firstDayOfWeek, dateStr, onSelected, onClose, echoComponent) {
	// member variables
	this.activeDiv = null;
...
Calendar.prototype.show = function () {
	...
	this.hideShowCovered();
        this.echoComponent._redrawScreen(this.echoComponent);
};
