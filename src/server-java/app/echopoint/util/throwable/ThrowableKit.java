...
/**
 * <code>ThrowableKit</code> provides helper methods for handling Exceptions
...
public class ThrowableKit
{
  ...
  /**
   * This method creates a Throwable derived object that will attach the root
   * cause if you are running under JDK 1.4 or later.
   * <p/>
   * This method makes it easier in dumps that find the root cause of a
   * problem.
   * <p/>
   ...
   * <p/>
   * This method allows you to use the new 1.4 exception chaining if
   * possible.
   *
  ...
  /**
   * Short hand method for ThrowableKit.makeThrowable(throwableClass,
   * null,cause);
   *
   * @param throwableClass - must be a class derived from Throwable.class
   * @param cause - the root cause Throwable
  ...
  /**
   * This method creates a RuntimeException that will attach the root cause if
   * you are running under JDK 1.4 or later.  This makes it easier in
   * Exception dumps that find the root cause of a problem.  If the
   * RuntimeException(String,Throwable) contructor is not available (JDK 1.3
   * or below) then the message + " : " + cause.toString() will be used and
   * the root cause Throwable stack trace will not be avalable.
   * <p/>
   * This method allows you to use the new 1.4 exception chaining if
   * possible.
   *
   ...
   * Throwable.  It emuluates the Throwable.initCause(Throwable) method found
   * in JDK 1.4.
   * <p/>
   * If this cant be done then nothing is attached, such as when you are
   * running under JDK 1.3
   * <p/>
   * <p>This method can be called at most once on an Throwable.  It is
   * generally called from within the constructor, or immediately after
   * creating the throwable.  If this throwable was created with {@link
   * Throwable#Throwable(Throwable)} or {@link Throwable#Throwable(String,Throwable)},
  ...
  public static Throwable attachCause( Throwable throwable, Throwable cause )
  {
    try
    {
      ...
      throwable = (Throwable) m.invoke( throwable, cause );
    }
  ...
  /**
   * This method will examine a Throwable and return a ThrowableDescriptor
   * that can then be used to output error details.
   * <p/>
  ...
  /**
   * Prints a textual description of a Throwable and its properties, causes
   * and stack traces.
   *
   ...
   *
   * @see ThrowableKit#printThrowableDescription(Throwable, PrintStream,
   *      boolean)
   */
  ...
  private static void printThrowableDescription0( ThrowableDescriptor desc, PrintStream out, int tabLevel, boolean indent )
  {
    ...
    for ( ThrowablePropertyDescriptor property : properties )
    {
      if ( !doneHeader )
      {
        doneHeader = true;
    ...
    for ( ThrowableDescriptor cause : causes )
    {
      if ( !doneHeader )
      {
        doneHeader = true;
      ...
      printThrowableDescription0( cause, out, tabLevel, indent );
    }
  }
